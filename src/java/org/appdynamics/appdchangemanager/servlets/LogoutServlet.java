...
public class LogoutServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Cookie groupCookie = null;
        
        Cookie[] cookies = request.getCookies();
        
        if( cookies != null){
            for(Cookie cookie: cookies){
                if(cookie.getName().equals(ACMS.USERNAME)){userCookie=cookie;}
                if(cookie.getName().equals(ACMS.ROLENAME)){groupCookie=cookie;}
            }
           
        }
        
        if(session != null && userCookie != null && groupCookie != null){
            log.log(Level.INFO,"Invalidating session");
            manageSessions.manageSession(session.getId(), userCookie.getValue(), groupCookie.getValue(), 2);
            session.invalidate();
